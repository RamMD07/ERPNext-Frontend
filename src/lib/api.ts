...
import { FrappeResponse, DocType, Document, User, ListViewSettings } from '../types/frappe';
...
class FrappeAPI {
  ...
  private setupInterceptors() {
    ...
    this.client.interceptors.request.use((config) => {
      if (this.csrf_token) {
        config.headers['X-Frappe-CSRF-Token'] = this.csrf_token;
      }
      return config;
    });
  ...
  async login(username: string, password: string): Promise<User> {
    ...
    if (response.data.message) {
      this.csrf_token = response.headers['x-frappe-csrf-token'];
